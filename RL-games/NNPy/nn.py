...
class NN():
	def __init__(self, structure, LEARNING_RATE, network=None):
	...
	def getCopy(self):
		copyStructure = self.structure.copy()
		copyNetwork = [layer.copy() for layer in self.network]

		return NN(copyStructure, self.LEARNING_RATE, copyNetwork)

	def getChild(self, mutateRate):
		# Create a mutated network array
		childNetwork = [layer.copy() for layer in self.network]
		for layerNum in range(len(self.structure)-1):
			for neuronNum in range(self.structure[layerNum+1]):

				#print(childNetwork[layerNum][neuronNum])

				childNetwork[layerNum][neuronNum] = np.array([
					self.getMutatedWeight(weight, mutateRate) 
					for weight in childNetwork[layerNum][neuronNum]])

		copyStructure = self.structure.copy()

		#print("old", self.network)
		#print("new", childNetwork)

		return NN(copyStructure, self.LEARNING_RATE, childNetwork)

	def getMutatedWeight(self, weight, mutateRate):
		return weight + (2*random.random()-1) * mutateRate
